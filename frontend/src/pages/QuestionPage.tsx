...
const QuestionHeader: React.FC<QuestionHeaderProps> = ({ question, publishedAnswerCount, currentVote }) => {
            ...
            <div className="flex-1">
              ...
              {question.issue_tags && question.issue_tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-6">
                  {question.issue_tags.map((tag) => (
                    <span key={tag} className="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-semibold bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 border border-blue-200 hover:border-blue-300 transition-colors">
                      #{tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-6 text-sm text-gray-600 border-t border-gray-200 pt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{publishedAnswerCount}</span>
                  <span>Candidate {publishedAnswerCount === 1 ? 'Answer' : 'Answers'}</span>
                </div>
              </div>
            </div>
          </div>
...
interface AnswerSidebarProps {
  ...
  onSelect: (answer: VideoAnswer) => void;
}

const AnswerSidebar: React.FC<AnswerSidebarProps> = ({ answers, selectedAnswerId, answersLoading, onSelect }) => (
    ...
    {answers.length > 0 ? (
          ...
          >
            <div className="flex items-start space-x-3">
              {answer.candidate?.photo_url ? (
                <img src={answer.candidate.photo_url} alt={answer.candidate.name} className="w-12 h-12 rounded-full object-cover border-2 border-gray-200 group-hover:border-blue-400 transition-colors" />
              ) : (
      ...
      !answersLoading && (
        <div className="bg-gradient-to-br from-gray-50 to-blue-50/30 rounded-xl p-8 text-center border-2 border-dashed border-gray-300">
          ...
          <p className="text-xs text-gray-600">Candidates will be notified of this question</p>
        </div>
      )
    )}
  </div>
...
const AnswerDetails: React.FC<AnswerDetailsProps> = ({ answer }) => (
  <div className="animate-fadeIn">
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-6">
      <VideoPlayer answer={answer} />
    </div>
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-6">
      <Link
        to={`/candidate/${answer.candidate_id}`}
      ...
      >
        ...
        <div>
          <p className="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors">
            {answer.candidate?.name || 'Unknown Candidate'}
          </p>
      ...
      <div className="p-6 space-y-6">
        {answer.rationale && (
          <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
            <h4 className="text-sm font-bold text-blue-900 mb-2">Rationale</h4>
            <p className="text-gray-700 leading-relaxed">{answer.rationale}</p>
          </div>
        )}
        {answer.implementation_plan && (
          <div className="p-4 bg-green-50 rounded-xl border border-green-100">
            <h4 className="text-sm font-bold text-green-900 mb-2">Implementation Plan</h4>
            <p className="text-gray-700 leading-relaxed">{answer.implementation_plan}</p>
          </div>
        )}
        {answer.tradeoff_acknowledged && (
          <div className="p-4 bg-yellow-50 rounded-xl border border-yellow-100">
            <h4 className="text-sm font-bold text-yellow-900 mb-2">Tradeoffs Acknowledged</h4>
            <p className="text-gray-700 leading-relaxed">{answer.tradeoff_acknowledged}</p>
          </div>
        )}
        {answer.measurement_criteria && (
          <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
            <h4 className="text-sm font-bold text-purple-900 mb-2">Success Metrics</h4>
            <p className="text-gray-700 leading-relaxed">{answer.measurement_criteria}</p>
          </div>
        )}
        {answer.values_statement && (
          <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
            <h4 className="text-sm font-bold text-indigo-900 mb-2">Values Statement</h4>
            <p className="text-gray-700 leading-relaxed">{answer.values_statement}</p>
          </div>
        )}
      </div>
    </div>
...
const QuestionPage: React.FC = () => {
  ...
  const { data: question, isLoading: questionLoading, error: questionError } = useQuestion(questionIdNum);
  const { data: videoAnswers, isLoading: answersLoading } = useQuestionVideoAnswers(questionIdNum);
      ...
      <Layout>
        <div className="container mx-auto px-4 py-8 max-w-5xl space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="space-y-3">
              {['sk-ans-0', 'sk-ans-1', 'sk-ans-2'].map((id) => <div key={id} className="skeleton h-20 rounded-xl" />)}
            </div>
            <div className="lg:col-span-3 skeleton rounded-2xl aspect-video" />
          </div>
        </div>
      </Layout>
    );
      ...
      <Layout>
        <div className="container mx-auto px-4 py-16 flex items-center justify-center">
          <div className="max-w-md w-full">
            <ErrorMessage title="Unable to Load Question" message="Failed to load question. Please try again." variant="card" />
          </div>
        </div>
      </Layout>
    );
      ...
      <div className="container mx-auto px-4 py-8 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            {selectedAnswer ? (
              <AnswerDetails answer={selectedAnswer} />
            ) : (
              <div className="bg-gradient-to-br from-gray-50 to-blue-50/30 rounded-2xl p-16 text-center border-2 border-dashed border-gray-300">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-3">Select a Candidate Answer</h3>
        ...
        {!isAuthenticated && (
          <div className="mt-8 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-8 shadow-xl text-white">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold mb-3">Want to Vote on This Question?</h3>
              ...
              <Link to="/login" className="inline-block px-8 py-4 bg-white text-blue-600 rounded-xl hover:bg-blue-50 transition-all font-bold shadow-lg hover:shadow-xl hover:scale-105 transform">
                Login to Vote
              </Link>
            </div>
          </div>
