...
const CandidateProfileHeader: React.FC<CandidateProfileHeaderProps> = ({ candidate, isCandidateUser }) => (
...
const CandidatePortal: React.FC<CandidatePortalProps> = ({ candidate, publishedCount, draftCount }) => (
  <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-8 mb-8 shadow-xl text-white">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <PortalStat label="Published Answers" value={publishedCount} />
      <PortalStat label="Draft Answers" value={draftCount} />
...
/* ── VideoAnswerCard ── */
interface VideoAnswerCardProps {
  answer: VideoAnswer;
  isSelected: boolean;
  index: number;
...
const VideoAnswerCard: React.FC<VideoAnswerCardProps> = ({ answer, isSelected, index, onSelect }) => (
  <button
    key={answer.id}
...
const VideoPlayerModal: React.FC<VideoPlayerModalProps> = ({ answer, onClose }) => (
...
interface CandidateInfoCardsProps {
  candidate: Candidate;
  isCandidateUser: boolean;
  publishedAnswers: VideoAnswer[];
}

const CandidateInfoCards: React.FC<CandidateInfoCardsProps> = ({ candidate, isCandidateUser, publishedAnswers }) => (
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex justify-between items-center p-3 bg-blue-50 rounded-xl">
          ...
          <span className="text-2xl font-bold text-blue-600">
            {Math.floor(publishedAnswers.reduce((sum, a) => sum + a.duration, 0) / 60)} min
          </span>
        </div>
      </div>
    </div>
...
const CandidatePage: React.FC = () => {
      ...
      <Layout>
        <div className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {['sk-stat-0', 'sk-stat-1', 'sk-stat-2', 'sk-stat-3'].map((id) => (
        ...
        <div className="container mx-auto px-4 py-16 flex items-center justify-center">
          <div className="max-w-md w-full">
            <ErrorMessage
              ...
              message="Failed to load candidate profile. Please try again."
              variant="card"
            />
          </div>
        </div>
  ...
  const publishedAnswers = videoAnswers?.filter((a: VideoAnswer) => a.status === 'published') || [];
  const draftAnswers = videoAnswers?.filter((a: VideoAnswer) => a.status === 'draft') || [];

  return (
    <Layout>
      <div className="container mx-auto px-4 py-8 animate-fadeIn">
        <CandidateProfileHeader candidate={candidate} isCandidateUser={isCandidateUser} />

        {isCandidateUser && (
          <CandidatePortal
            candidate={candidate}
            publishedCount={publishedAnswers.length}
            draftCount={draftAnswers.length}
          />
        )}
        ...
        <div>
          ...
          {publishedAnswers.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {publishedAnswers.map((answer: VideoAnswer, index: number) => (
                <VideoAnswerCard
                  key={answer.id}
                  answer={answer}
              ...
              <div className="bg-gradient-to-br from-gray-50 to-blue-50/30 rounded-2xl p-16 text-center border-2 border-dashed border-gray-300">
                ...
                <p className="text-lg text-gray-600 mb-6">
                  {isCandidateUser
                    ...
                    : 'This candidate has not recorded any answers yet.'}
                </p>
                {isCandidateUser && (
                  <Link
                    to={`/candidate/${candidate.id}/record`}
          ...
          {isCandidateUser && draftAnswers.length > 0 && (
            <div className="mt-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {draftAnswers.map((answer: VideoAnswer) => (
                  <div
                    key={answer.id}
        ...
        {selectedAnswer && (
          <VideoPlayerModal answer={selectedAnswer} onClose={() => setSelectedAnswer(null)} />
        )}
